...
Sub Class_Globals
	...
	Public ResMap				As Map
	Public ProMap				As Map
End Sub
...
Public Sub PostBytes(Link As String, Data() As Byte)
	...
	CallSubDelayed2(MNCUtils2Service, "SubmitJob", Me)
End Sub
...
Public Sub Release
	File.Delete(MNCUtils2Service.TempFolder, taskId)
End Sub
...
Public Sub GetString2(Encoding As String) As String
	...
	Try
		tr.Initialize2(File.OpenInput(MNCUtils2Service.TempFolder, taskId), Encoding)
	Catch
		Return Null
	End Try
...
Public Sub GetString3(NamaDir As String, Namafile As String) As Boolean
	Dim tr As TextReader
	
	Try
		tr.Initialize(File.OpenInput(MNCUtils2Service.TempFolder, taskId))
	Catch
		Return False
	End Try
	...
	res = tr.ReadAll
	tr.Close
	File.WriteString(NamaDir, Namafile, res)
	Return True
End Sub
...
Public Sub GetBitmap As Bitmap
	Dim b As Bitmap
'	b  = LoadBitmap (MNCUtils2Service.TempFolder, taskId)
	b.Initialize(MNCUtils2Service.TempFolder, taskId)
	Return b
End Sub

Public Sub GetInputStream As InputStream
	Dim In As InputStream
	In = File.OpenInput(MNCUtils2Service.TempFolder, taskId)
	Return In
End Sub

Public Sub GetBitmapSampled(Width As Int, Height As Int) As Bitmap
	Dim b As Bitmap
	b = LoadBitmapSample(MNCUtils2Service.TempFolder, taskId, Width, Height)
	Return b
End Sub
